...
async def run(args):
    ...
    environment["PYTHONPATH"] = os.pathsep.join(
        [str(local_packages), str(repository), str(repository / "src")]
    )
    ...
    async with client:
        ...
        tool_name = "compute_strut_metrics"
        if not args.metrics_only:
            if args.thresholds_json is None:
                raise ValueError(
                    "--thresholds-json is required unless --metrics-only is used"
                )
            tool_name = "run_thin_thick_bent_pipeline"
            arguments["thresholds_json"] = str(args.thresholds_json.resolve())
