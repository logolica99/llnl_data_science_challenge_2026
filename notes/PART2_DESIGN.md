...
| `compute_registration_qa` | Three explicit outputs: (1) production image QA on the selected coordinate graph over all 10,206 node records and 18,468 edges, including X/Y/Z bins and a color-coded localized/stable-coarse/fallback overlay; (2) a **coarse-capture gate** using displacement and repeatability read from the hashed localization report, followed by image-support checks on the 20 %-padded ROIs; and (3) a stricter metrology/direct-corridor gate that requires an artifact-backed absolute-registration uncertainty. The MCP boundary accepts no agent-selected uncertainty scalar. Challenge-aligned mode therefore makes no direct-metrology claim. |
